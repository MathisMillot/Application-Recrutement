...
**OffreEmploi**(<u>id_offre</u> `PK`, statut, date_expiration, description, nb_prises_demandes, siren_organisation `FK`)
- `siren_organisation` → Organisation.siren

**Candidature**(<u>id_candidature</u> `PK`, date, id_candidat `FK`, id_offre `FK [0..1]`)
- `id_candidat` → Candidat.id_user
- `id_offre` → OffreEmploi.id_offre *(nullable)*

**DocumentsCandidature**(<u>id_dossier</u> `PK`, nom, id_candidature `FK`)
